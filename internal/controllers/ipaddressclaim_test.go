...
var _ = Describe("IPAddressClaimReconciler", func() {
	...
	BeforeEach(func() {
		...
		mockInfobloxClient.EXPECT().GetHostConfig().Return(&infoblox.HostConfig{}).AnyTimes()
		...
		ibSecret := corev1.Secret{
			ObjectMeta: metav1.ObjectMeta{Name: instanceName + "-secret", Namespace: "default"},
			StringData: map[string]string{"username": "admin", "password": "pass"},
		}
		Expect(k8sClient.Create(context.Background(), &ibSecret)).To(Succeed())
	})

	AfterEach(func() {
		deleteObj(&v1alpha1.InfobloxInstance{}, instanceName, "")
		deleteObj(&corev1.Secret{}, instanceName+"-secret", "default")
	})
	...
	When("a new IPAddressClaim is created", func() {
		...
		When("the referenced namespaced pool exists", func() {
			...
			BeforeEach(func() {
				setLocalMock(ibmock.NewMockClient(mockCtrl))
			...
			When("a claim is deleted", func() {
				...
				It("should prevent deletion of claims", func() {
					...
					Eventually(func() bool {
						err := k8sClient.Get(context.Background(), client.ObjectKeyFromObject(&pool), &pool)
						return err == nil && annotations.HasPaused(&pool)
...
func deleteNamespacedPool(name, namespace string) {
	...
	EventuallyWithOffset(1, Get(&pool)).WithTimeout(30 * time.Second).WithPolling(200 * time.Millisecond).Should(Not(Succeed()))
}
...
func deleteClaim(name, namespace string) {
	...
	EventuallyWithOffset(1, Get(&claim)).WithTimeout(30 * time.Second).WithPolling(200 * time.Millisecond).Should(Not(Succeed()))
}
...
func setLocalMock(m *ibmock.MockClient) {
	mockMu.Lock()
	localInfobloxClientMock = m
	mockMu.Unlock()
}
