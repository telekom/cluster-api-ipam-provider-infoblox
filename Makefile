...
test-infoblox: manifests generate fmt vet envtest ## Run infoblox instance tests - instance is required to be configured.
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) -p path)" go test $(shell go list ./... | grep /infoblox) -coverprofile cover-infoblox.out
...
coverage-check: test ## Enforce minimum coverage threshold; runs tests first.
...
.PHONY: test-coverage
test-coverage: coverage-check ## Run tests and enforce minimum coverage.
